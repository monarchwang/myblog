				...
				<div class="ins-section-wrapper">
					<div class="ins-section-container">
						<section class="ins-section"
							...
							<div class="ins-selectable ins-search-item" v-for="summary in dataSummary.articleDtos">
						...
						<section class="ins-section" v-show="dataSummary.tags && dataSummary.tags.length>0">
							...
							<div class="ins-selectable ins-search-item" v-for="tag in dataSummary.tags">
								<header>
	...
	export default {
		...
		watch: {
			keywords(val){
				...
				this.dataSummary.articleDtos = articleDtos;
				this.dataSummary.tags = tags;
			}
		},
		methods: {
			goto(item) {
			...
			searchFocus() {
				...
				this.dataSummary.articleDtos = this.$globalData.dataSummary.articleDtos;
				this.dataSummary.tags = this.$globalData.dataSummary.tags;
			},
			closeModal() {
			...
			getSummary(){
				Api.getSummary().then(res => {
					if(res.data){
						...
						this.dataSummary.articleDtos = res.data.articleDtos.split(0, 5);
						this.dataSummary.tags = res.data.tags.split(0, 5);
					}
				}).catch(error => {
					console.log(error)
				})
